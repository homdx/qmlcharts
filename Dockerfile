...
RUN mkdir /app && cd app && export ANDROID_TARGET_SDK_VERSION=28 && \
    ...
    echo 'fix missing libc++_shared.so' && echo cp -vf /android-ndk-r20/sources/cxx-stl/llvm-libc++/libs/armeabi-v7a/libc++_shared.so android-build/libs/armeabi-v7a/ && \
    echo mkdir /android-ndk-r20/sources/cxx-stl/gnu-libstdc++/4.9/libs/armeabi-v7a -pv && \
    echo build-android-gradle-project qmlchart.pro --debug && \
