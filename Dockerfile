...
RUN mkdir /android-ndk-r20/sources/cxx-stl/gnu-libstdc++/4.9/libs/armeabi-v7a -pv && \
    wget https://github.com/urho3d/android-ndk/raw/master/sources/cxx-stl/gnu-libstdc%2B%2B/4.9/libs/armeabi-v7a/libgnustl_shared.so -O /android-ndk-r20/sources/cxx-stl/gnu-libstdc++/4.9/libs/armeabi-v7a/libgnustl_shared.so
...
RUN mkdir /app && cd app && export ANDROID_TARGET_SDK_VERSION=28 && \
    ...
    echo 'fix missing libc++_shared.so' && cp -vf /android-ndk-r20/sources/cxx-stl/llvm-libc++/libs/armeabi-v7a/libc++_shared.so android-build/libs/armeabi-v7a/ && \
    mkdir /android-ndk-r20/sources/cxx-stl/gnu-libstdc++/4.9/libs/armeabi-v7a -pv && \
